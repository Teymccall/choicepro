...
const FloatingNav = () => {
  ...
  return (
    <nav 
      className="fixed bottom-0 left-0 right-0 bg-white/85 dark:bg-gray-900/85 backdrop-blur-2xl border-t border-gray-100/80 dark:border-gray-800 z-50 md:hidden transition-transform duration-300 ease-in-out shadow-[0_-8px_30px_rgba(0,0,0,0.04)]"
      style={{
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.path}
              to={item.path}
              className={`flex flex-col items-center justify-center flex-1 h-full relative group transition-all duration-300 ${isActive ? '-translate-y-0.5' : 'opacity-80 hover:opacity-100'}`}
            >
              {/* Icon Container */}
              <div className="relative mb-1">
                ...
                {hasNotifications && (
                  <span className="absolute -top-1 -right-1.5 flex h-4 w-4">
                    ...
                    <span className="relative inline-flex rounded-full h-4 w-4 bg-red-500 text-white text-[9px] items-center justify-center font-bold shadow-sm border border-white">
