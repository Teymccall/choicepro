...
const Navigation = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(chatsRef, (snapshot) => {
      ...
      let unreadCount = 0;
      const currentPairingId = [user.uid, partner.uid].sort().join('_');
      const currentDirectChatId = `direct_chat_${currentPairingId}`;

      Object.entries(data).forEach(([topicId, chat]) => {
        // IMPORTANT: Only count messages from the current active partnership
        // Match the modern isolated chat, or the legacy shared chat
        if (topicId !== 'direct_chat' && topicId !== currentDirectChatId) return;
        ...
        const unreadMessages = Object.entries(chat).filter(([messageId, message]) => {
          if (messageId === 'typing') return false;
          if (!message || typeof message !== 'object') return false;
          
          // Only count if it's explicitly from the current active partner
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(notificationsRef, (snapshot) => {
      ...
      if (!isInitialLoad) {
        // Trigger high priority toasts for screenshot detection events
        notificationsList.forEach(notification => {
          if (notification.type === 'screenshot_detected' && !toastedIds.has(notification.id)) {
            toast.error(
              <div className="flex flex-col gap-1">
                <span className="font-bold text-lg">⚠️ Screenshot Alert!</span>
                <span>{notification.message || 'Your partner attempted to take a screenshot'}</span>
              </div>,
              {
                duration: 6000,
                position: 'top-center',
        ...
        // Initialize the toastedIds set with existing notifications on load
        Object.keys(data).forEach(id => toastedIds.add(id));
      }
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="hidden sm:flex sm:items-center sm:space-x-2">
              {/* Call Buttons */}
              {partner && (
                <div className="flex items-center space-x-1 border-r border-gray-200 dark:border-gray-700 pr-2 mr-1">
                  <button
                    onClick={() => {
                      if (!webRTC) return;
                      // Prevent starting new call if one is already active
                      if (webRTC.callStatus && webRTC.callStatus !== 'idle') return;
              ...
              <div className="relative notifications-container">
                <button
                  ...
                  className="p-1.5 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-200 relative"
                >
              ...
              <div className="flex items-center space-x-2">
                ...
                <button
                  ...
                  className="inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-md text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 transition-all duration-200"
                >
                ...
                {partner && (
                  <div className="flex items-center space-x-1 border-r border-gray-200 dark:border-gray-700 pr-3">
                    <button
                      ...
                      disabled={webRTC?.callStatus && webRTC.callStatus !== 'idle'}
                      className="p-1.5 rounded-full hover:bg-green-100 dark:hover:bg-green-900/30"
                    >
                    ...
                    <button
                      ...
                      disabled={webRTC?.callStatus && webRTC.callStatus !== 'idle'}
                      className="p-1.5 rounded-full hover:bg-blue-100 dark:hover:bg-blue-900/30"
                    >
