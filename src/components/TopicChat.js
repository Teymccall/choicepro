...
const TopicChat = ({ topic, onClose }) => {
  ...
  const [disappearingTimer, setDisappearingTimer] = useState(null); // null, 10, 60, 3600, 86400
  ...
  const isBroadcastingTypingRef = useRef(false);
  ...
  const handleMessageChange = (e) => {
    ...
    if (message.trim()) {
      if (!isBroadcastingTypingRef.current) {
        console.log('Started typing, broadcasting to network');
        updateTypingStatus(true);
        isBroadcastingTypingRef.current = true;
      }

      typingTimeoutRef.current = setTimeout(() => {
        console.log('Typing timeout, setting typing status to false');
        updateTypingStatus(false);
        isBroadcastingTypingRef.current = false;
  ...
  const handleDeleteMessage = async (messageId, deleteForEveryone = false) => {
    try {
      if (deleteForEveryone) {
        // Mark message as deleted for everyone — both partners see it removed
        ...
        // Delete for me only — store in per-user deleted messages node
        const deletedRef = ref(rtdb, `deletedMessages/${user.uid}/${topic.id}/${messageId}`);
  ...
  const handleSendMessage = async (e) => {
    ...
    if (editingMessage) {
      ...
      try {
        ...
        const chatRef = ref(rtdb, `topicChats/${topic.id}`);
        // Only allow disappearing on images/videos
        const isDisappearingEligible = selectedFile && !selectedFile.type.startsWith('audio');
        ...
        const messageDataWithMedia = {
          ...
          ...(mediaData && {
            media: {
              ...
              ...(isDisappearingEligible && disappearingTimer ? { disappearingTimer } : {})
            }
          })
        };

        // Reset timer after message is built
        ...
        // ============ VERCEL CUSTOM FCM PUSH NOTIFICATION ============ //
        try {
          if (partner?.uid && user?.displayName) {
            // Retrieve partner's FCM tokens
            ...
            if (tokensSnapshot.exists()) {
              ...
              // Only blast push if they have active devices
              if (tokens.length > 0) {
                // We'll iterate their registered tokens to hit their devices
                for (const token of tokens) {
                  fetch(window.location.origin + '/api/send-push', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                      token: token,
                      title: `New Message from ${user.displayName}`,
                      body: messageDataWithMedia.text || (messageDataWithMedia.media ? 'Sent an attachment' : 'Sent a message'),
                      data: {
                        type: 'chat_message',
                        topicId: topic.id
                      }
                    })
                  }).catch(e => console.log('Background push delivery failed (non-critical):', e));
                }
              }
            }
          }
        } catch (pushError) {
          console.warn('Push notification delivery handled softly:', pushError);
        }
        // ============================================================ //
  ...
  const stopRecording = ({ autoSend = false } = {}) => {
    ...
    if (recorder && recorder.state !== 'inactive') {
      ...
      // Fallback: If onstop fails to fire due to iOS bugs, clear the UI after 500ms
      setTimeout(() => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(partnerRef, (snapshot) => {
      if (snapshot.exists()) {
        const data = snapshot.val();
        // Update local partner data state with profile fields (do NOT overwrite presence fields)
        setPartnerData(current => ({
          ...
          status: data.status,
          // Preserve real-time presence fields
        ...
        {showMediaMenu && (
          <>
            ...
            <div className="fixed bottom-0 left-0 right-0 z-50 animate-slide-up">
              <div className="bg-white/98 dark:bg-gray-900/98 backdrop-blur-2xl rounded-t-3xl shadow-2xl border-t border-gray-200/30 dark:border-gray-700/30 mx-auto max-w-lg">
                {/* Handle bar */}
                ...
                <div className="flex items-center justify-center gap-8 px-6 py-5">
                  ...
                  >
                    <div className="w-14 h-14 bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-500/25 group-hover:scale-110 group-active:scale-95 transition-transform duration-200">
                      <PhotoIcon className="h-6 w-6 text-white" strokeWidth={2} />
                    </div>
                    <span className="text-[11px] font-semibold text-gray-600 dark:text-gray-400">Gallery</span>
                  </button>
                  ...
                  <button
                    ...
                    className="flex flex-col items-center gap-2 group"
                  >
                    <div className="w-14 h-14 bg-gradient-to-br from-purple-500 to-purple-600 rounded-2xl flex items-center justify-center shadow-lg shadow-purple-500/25 group-hover:scale-110 group-active:scale-95 transition-transform duration-200">
                      <CameraIcon className="h-6 w-6 text-white" strokeWidth={2} />
                    </div>
                    <span className="text-[11px] font-semibold text-gray-600 dark:text-gray-400">Camera</span>
                  </button>
                  ...
                  <button
                    ...
                    className="flex flex-col items-center gap-2 group"
                  >
                    <div className="w-14 h-14 bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-2xl flex items-center justify-center shadow-lg shadow-emerald-500/25 group-hover:scale-110 group-active:scale-95 transition-transform duration-200">
                      <DocumentIcon className="h-6 w-6 text-white" strokeWidth={2} />
                    </div>
                    <span className="text-[11px] font-semibold text-gray-600 dark:text-gray-400">File</span>
                  </button>
                </div>
