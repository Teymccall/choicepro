...
const SplashScreen = ({ onComplete, authReady = false }) => {
  ...
  // Only begin fade-out when BOTH animation is done AND auth is ready
  useEffect(() => {
  ...
  }, [animationDone, authReady, onComplete, reduceMotion]);

  // Progress bar animation — stalls at 90% if auth isn't ready yet
  useEffect(() => {
    if (reduceMotion) {
      setProgress(authReady ? 100 : 90);
      return undefined;
    }

    const maxProgress = authReady ? 100 : 90;
    const totalSteps = Math.ceil(DISPLAY_DURATION_MS / 80);
    let currentStep = 0;

    const interval = setInterval(() => {
      currentStep += 1;
      const rawPercentage = Math.round((currentStep / totalSteps) * 100);
      const percentage = Math.min(maxProgress, rawPercentage);
  ...
  }, [reduceMotion, authReady]);

  // When auth becomes ready while stalled at 90%, quickly fill to 100%
  useEffect(() => {
    if (authReady && progress >= 85 && progress < 100) {
        ...
        <div className="flex flex-col items-center text-center space-y-8">
          ...
          <div className="w-full bg-white/10 border border-white/10 rounded-3xl p-6 backdrop-blur-lg shadow-2xl">
            <p className="text-xs uppercase tracking-[0.3em] text-white/60 mb-4">
              {progress < 100 ? 'Loading workspace' : 'Ready'}
            </p>
            <div className="h-1.5 w-full rounded-full bg-white/20 overflow-hidden">
              <div
                className="h-full rounded-full bg-white transition-all duration-150 ease-linear"
