        ...
        for (const change of snapshot.docChanges()) {
          if (change.type === 'modified') {
            ...
            if (request.status === 'declined') {
              ...
              // ✅ Immediately update partner state so the UI reflects the
              // connection without requiring a page refresh.
              try {
                const partnerDoc = await getDoc(doc(db, 'users', request.recipientId));
                if (partnerDoc.exists()) {
                  setPartner(partnerDoc.data());
                  console.log('✅ Partner state updated after request accepted:', request.recipientId);
                }
              } catch (fetchErr) {
                console.warn('Could not fetch partner after accept (non-fatal):', fetchErr.message);
              }
            }
          }
        }
                ...
                if (userData.partnerId) {
                  try {
                    ...
                    if (partnerDoc.exists()) {
                      ...
                      setPartner(newPartnerData);
                      console.log('✅ Partner set from userDocument listener:', newPartnerData.displayName);

                      // Set up RTDB connection entry so presence monitoring works
                      ...
                      try {
                        ...
                        if (!connectionSnapshot.exists() || connectionSnapshot.val()?.partnerId !== userData.partnerId) {
                          ...
                          console.log('✅ RTDB connection entry created for partner:', userData.partnerId);
                        }
                      ...
                      // Re-initialize full database listeners if this is a NEW partner connection
                      // Check if we weren't already listening to this exact partner
                      try {
                        const presenceRefNode = ref(rtdb, `presence/${userData.partnerId}`);
                        const presenceUnsub = onValue(presenceRefNode, (presenceSnapshot) => {
                           if (presenceSnapshot.exists()) {
                             const presenceData = presenceSnapshot.val();
                        ...
                        listenerCleanups.current.push(presenceUnsub);
                      } catch(presenceErr) {
                         console.warn('Could not set up presence listener:', presenceErr);
                      }
                      } catch (rtdbErr) {
                        console.warn('Could not set RTDB connection/presence (non-fatal):', rtdbErr.message);
                      }
                    }
                  } catch (error) {
